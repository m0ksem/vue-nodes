...
<template>
  <DemoNode :title="node.title">
    <template #inputs>
      <DemoButton color="red" @register-point="registerPoint(node, 'in')($event)" @circle-click="$emit('connect-to', 'in')">
        <DemoInput v-model="node.variable.value"/>
      </DemoButton>
      <DemoButton>
         <DemoInput v-model="node.variable.type"/>
      </DemoButton>
      <DemoButton>
         <DemoInput v-model="node.variable.name"/>
      </DemoButton>
    </template>

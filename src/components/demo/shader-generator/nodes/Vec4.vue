...
<template>
  <DemoNode :title="node.title">
    <template #inputs>
      <DemoButton color="red" @register-point="registerPoint(node, 'x')($event)" @circle-click="$emit('connect-to', 'x')">
        <DemoInput v-model="node.points['x'].value"/>
      </DemoButton>
      <DemoButton color="red" @register-point="registerPoint(node, 'y')($event)" @circle-click="$emit('connect-to', 'y')">
        <DemoInput v-model="node.points['y'].value"/>
      </DemoButton>
      <DemoButton color="red" @register-point="registerPoint(node, 'z')($event)" @circle-click="$emit('connect-to', 'z')">
        <DemoInput v-model="node.points['z'].value"/>
      </DemoButton>
      <DemoButton color="red" @register-point="registerPoint(node, 'w')($event)" @circle-click="$emit('connect-to', 'w')">
        <DemoInput v-model="node.points['w'].value"/>
      </DemoButton>
    </template>
